...
async def fabricar_video(tema, video_id):
    ...
    # 1. GUIONISTA DE ÉLITE (Gemini)
    api_key = os.environ.get("GEMINI_API_KEY")
    genai.configure(api_key=api_key)
    model = genai.GenerativeModel('gemini-1.5-flash')
    # Pedimos un guion con estructura de retención viral
    prompt = f"Escribe un guion para un video de 60 segundos sobre {tema}. Estilo: Documental de suspenso. Empieza con una pregunta que nadie pueda ignorar. Usa frases de máximo 5 palabras. Sin asteriscos."
    ...
    with open(f_voz, "wb") as f:
        async for chunk in communicate.stream():
            if chunk["type"] == "audio": f.write(chunk["data"])
                ...
                # Subtítulos: Blanco puro, borde negro grueso, sombra y posición central
    ...
    # 3. PEXELS API (Búsqueda Cinematográfica)
    pex_key = os.environ.get("PEXELS_API_KEY")
    headers = {"Authorization": pex_key}
    print("🎥 Extrayendo metraje 4K...")
    # Buscamos términos relacionados para mayor variedad
    r = requests.get(f"https://api.pexels.com/videos/search?query={tema}&per_page=3&orientation=portrait", headers=headers).json()
    ...
    # 5. RENDERIZADO "CINEMA MASTER" (El secreto de la envidia)
    print("🏗️ Renderizando pieza maestra...")
    # Filtros: 
    # 1. unsharp: para dar nitidez extrema
    # 2. curves: para dar un tono azulado/oscuro cinematográfico
    # 3. noise: grano de película para realismo
    # 4. vignette: para centrar la atención
    cmd = (
    ...
    print(f"🔱 EL MUNDO NO ESTÁ PREPARADO PARA ESTO")
