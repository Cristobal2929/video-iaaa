...
async def fabricar_video(tema, video_id):
    print(f"🎬 PRODUCCIÓN ÉPICA CON SUBS: {tema}")
    os.makedirs("static", exist_ok=True)
    
    f_voz = "voz_temp.mp3"
    f_subs = "subs_temp.vtt"
    f_final = f"static/{video_id}.mp4"

    # 1. GUIONISTA DE ÉLITE (Gemini)
    api_key = os.environ.get("GEMINI_API_KEY")
    guion_base = f"¿Sabías que el misterio de {tema} es mucho más profundo de lo que imaginas? Prepárate para descubrir una verdad que ha estado oculta durante siglos."
    try:
    ...
    # 2. VOZ Y GENERACIÓN DE SUBTÍTULOS (Sincronización perfecta)
    ...
    # 3. GENERAR 3 IMÁGENES DISTINTAS (Secuencia)
    for i in range(1, 4):
    ...
    # 4. MONTAJE MAESTRO (3 Imágenes + Zoom + Subtítulos Amarillos)
    print("🎥 Montando minidocumental...")
    # Estilo: Amarillo brillante, borde negro grueso, tamaño grande
    ...
    # Comandos FFmpeg con filtros encadenados correctamente
    # Aplicamos zoom a cada imagen y luego las unimos
    filtro_v = (
    ...
    comando = (
        ...
        f'-filter_complex "{filtro_v}" -map "[v]" -map 3:a '
        f'-c:v libx264 -pix_fmt yuv420p -profile:v baseline -level 3.0 -movflags +faststart -shortest {f_final}'
    )
    os.system(comando)
    ...
    os.remove(f_voz)
    os.remove(f_subs)
    print(f"✅ VÍDEO CON SUBTÍTULOS LISTO")
